 ...
 * 
 * This class represents an immutable instruction in its final decoded form,
 * with all fields
...
public final class InstructionFormat {
    ...
    private static void validateImmediate(int imm, int bits, boolean signed) {
        // Efficient range calculation for immediate (signed goes from 0 to 2^(bits-1)-1
        // and unsigned goes from -(2^(bits-1)) to -1)
        int min = signed ? -(1 << (bits - 1)) : 0;
        int max = signed ? (1 << (bits - 1)) - 1 : (1 << bits) - 1;
        if (imm < min || imm > max) {
            throw new IllegalArgumentException("Immediate must be " + bits + "-bit " + (signed ? "signed" : "unsigned")
                    + " with " + min + " to " + max + ", got: " + imm);
        }
    }
    ...
    public boolean equals(Object obj) {
        if (this == obj)
            return true;
        if (!(obj instanceof InstructionFormat))
            return false;

        InstructionFormat other = (InstructionFormat) obj;
        return opcode == other.opcode && regA == other.regA && regB == other.regB && regC == other.regC
