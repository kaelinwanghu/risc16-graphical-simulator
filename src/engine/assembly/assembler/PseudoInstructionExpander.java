 ...
 * 
 * This MUST run BEFORE Pass 1 (before labels are stored) to prevent wrong address pointing from expansion
 * 
...
public final class PseudoInstructionExpander {
    ...
    private static List<Token> expandOne(Token token) {
        ...
        if (operands.length != expected) {
            throw new AssemblyException(lineNumber, operation.toUpperCase() + " requires " + expected + " operand(s), got " + operands.length, originalLine, AssemblyError.ErrorType.INVALID_OPERAND);
        }
        ...
        switch (operation) {
            case "nop":
                // NOP -> ADD R0, R0, R0
                result.add(new Token(lineNumber, token.getLabel(), "add", new String[]{"r0", "r0", "r0"}, originalLine));
                break;
            ...
            case "lli":
                ...
                if (immLLI == null) {
                    throw new AssemblyException(lineNumber, "LLI immediate must be a number, not a label", originalLine, AssemblyError.ErrorType.INVALID_OPERAND);
                }
                
                result.add(new Token(lineNumber, token.getLabel(), "addi", new String[]{operands[0], operands[0], String.valueOf(immLLI & ImmediateRanges.LLI_MASK)}, originalLine));
                break;
            ...
            case "movi":
                ...
                if (immMOVI == null) {
                    throw new AssemblyException(lineNumber, "MOVI immediate must be a number, not a label", originalLine, AssemblyError.ErrorType.INVALID_OPERAND);
                }
                
                if (!ImmediateRanges.isValidMOVI(immMOVI)) {
                    throw new AssemblyException(lineNumber, "MOVI immediate " + immMOVI + " out of range [" +  ImmediateRanges.MOVI_MIN + ", " + ImmediateRanges.MOVI_MAX + "]", originalLine, AssemblyError.ErrorType.INVALID_IMMEDIATE);
                }
                
                int upper10 = immMOVI >> ImmediateRanges.LUI_SHIFT;
                int lower6 = immMOVI & ImmediateRanges.LLI_MASK;
                
                // First instruction: LUI (gets the label)
                result.add(new Token(lineNumber, token.getLabel(), "lui", new String[]{operands[0], String.valueOf(upper10)}, originalLine));
                
                // Second instruction: ADDI (no label)
                result.add(new Token(lineNumber, null, "addi", new String[]{operands[0], operands[0], String.valueOf(lower6)}, originalLine));
                break;
                
            default:
                throw new AssemblyException(lineNumber, "Unknown pseudo-instruction: " + operation, originalLine, AssemblyError.ErrorType.INVALID_OPCODE);
        }
