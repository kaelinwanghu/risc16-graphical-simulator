...
public class StorageSettingsDialog extends JDialog {
	...
	private JCheckBox enableDebugging;
	private InputBox snapshotLimit;
	...
	public StorageSettingsDialog(Simulator simulator, StorageViewer storageViewer, EngineFacade engineFacade) {
		...
		Border b2 = BorderFactory.createTitledBorder(null, "Execution Settings", TitledBorder.LEFT, TitledBorder.TOP,
				new Font("Consolas", Font.PLAIN, 19), Color.RED);
		JPanel executionPanel = new JPanel(new GridLayout(2, 1, 0, 5));
		executionPanel.setBorder(BorderFactory.createCompoundBorder(b2, BorderFactory.createEmptyBorder(5, 0, 5, 0)));
		executionPanel.add(instructionLimit);

		Border b3 = BorderFactory.createTitledBorder(null, "Memory Viewer", TitledBorder.LEFT, TitledBorder.TOP,
				new Font("Consolas", Font.PLAIN, 19), Color.RED);
		JPanel memViewPanel = new JPanel(new GridLayout(3, 1, 0, 5));
		memViewPanel.setBorder(BorderFactory.createCompoundBorder(b3, BorderFactory.createEmptyBorder(5, 0, 5, 0)));
		...
		JPanel checkboxPanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 0, 5));
		checkboxPanel.add(enableDebugging);

		snapshotLimit = new InputBox("Snapshot Limit", 180, 10, "(max states)");
		snapshotLimit.setInput(engineFacade.getDebugManager().getSnapshotLimit());
	...
	private void apply() {
		...
		boolean debugEnabled = enableDebugging.isSelected();
		engineFacade.getDebugManager().setEnabled(debugEnabled);
		try {
			int snapLimit = snapshotLimit.getValue();
			if (snapLimit < 1 || snapLimit > 10000) {
				simulator.messageDialog.showError("Snapshot limit must be between 1 and 10,000");
				return;
			}
			engineFacade.getDebugManager().setSnapshotLimit(snapLimit);
		} catch (Exception ex) {
			simulator.messageDialog.showError("Invalid snapshot limit");
			return;
		}

		// Apply instruction limit (works of memory change)
		engineFacade.setInstructionLimit(limit);
		simulator.updateDebugVisibility();
